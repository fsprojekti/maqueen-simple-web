...
uint16_t DFRobot_MaqueenPlus::getGrayscale(ePatrol sensor)
{
...
uint16_t* DFRobot_MaqueenPlus::getGrayscale()
{
static uint16_t patrol_AD[5];
  uint8_t buf[10];
  readReg(0x1E, buf, 10);
  patrol_AD[0] = (buf[0] << 8) | buf[1];
  patrol_AD[1] = (buf[2] << 8) | buf[3];
  patrol_AD[2] = (buf[4] << 8) | buf[5];
  patrol_AD[3] = (buf[6] << 8) | buf[7];
  patrol_AD[4] = (buf[8] << 8) | buf[9];
  return patrol_AD;
}
